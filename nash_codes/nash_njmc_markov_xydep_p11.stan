...
transformed parameters{
  ...
  for (m in 1:M){
    ...
    p11[m] =  1/(1+exp(-etacr[m])); // transform between 0 and 1
    corr[m]= (p11[m]-pwet[m])/(1-pwet[m]);
...
generated quantities {
  ...
  // }
    ...
    for (j in 2:Nt) {
      if (y[m, j-1] < 1E-6){ // previous is a zero
          if (y[m,j] < 1E-6){ // now ia a zero
            log_lik[m] = log_lik[m] + bernoulli_lpmf(0 | p01[m]);
          } else {
            log_lik[m] = log_lik[m] + bernoulli_lpmf(1 | p01[m]);
          }
      } else { // previous is a one
          if (y[m,j] < 1E-6){ // mow is a zero
            log_lik[m] = log_lik[m] + bernoulli_lpmf(0 | p11[m]);
          } else {
            log_lik[m] = log_lik[m] + bernoulli_lpmf(1 | p11[m]);
          }
      }
    }
  }
